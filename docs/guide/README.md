...
- 设计或评审 Prisma seed，判断升级时是否会覆盖线上数据：
  - [环境初始化与基线](./environment-bootstrap.md) 中的 “Seed 升级安全规则”
- 理解当前底座有哪些正式已落地功能，以及平台侧 / 租户侧边界、固定菜单结构、角色模板与能力目录职责：
  - [底座目标态说明](./base-project-guide.md)
- 核对初始化后哪些功能应可直接验收，包括日志、个人中心和改密：
  - [环境初始化与基线](./environment-bootstrap.md)
...
- 涉及 seed 设计、升级覆盖语义和重复执行安全性时，统一以 [环境初始化与基线](./environment-bootstrap.md) 中的 Seed 规则为准。
