...
export type UserEssentialProperties = Readonly<Required<{ id: string; username: string; domain: string; nickName: string; status: Status }>>;
export type UserOptionalProperties = Readonly<Partial<{ password: string; avatar: string | null; email: string | null; phoneNumber: string | null }>>;
export type UserRelationProperties = Readonly<{ roleIds: string[] }>;
export type UserProperties = UserEssentialProperties & Required<UserOptionalProperties> & UserRelationProperties;
export type UserCreateProperties = Omit<UserProperties, 'roleIds'> & CreationAuditInfoProperties;
export type UserUpdateProperties = Omit<UserProperties, 'roleIds' | 'password' | 'domain'> & CreationAuditInfoProperties;
...
export class UserReadModel extends UpdateAuditInfo {
  ...
  @ApiProperty({ description: 'Domain associated with the user' }) domain: string;
