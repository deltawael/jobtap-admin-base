...
export class UserController {
  ...
  async page(@Query() queryDto: PageUsersDto): Promise<ApiRes<PaginationResult<UserProperties>>> {
    const query = new PageUsersQuery({ current: queryDto.current, size: queryDto.size, username: queryDto.username, nickName: queryDto.nickName, status: queryDto.status });
    const result = await this.queryBus.execute<PageUsersQuery, PaginationResult<UserProperties>>(query);
  ...
  async createUser(@Body() dto: UserCreateDto, @Request() req: any): Promise<ApiRes<null>> {
    await this.commandBus.execute(new UserCreateCommand(dto.username, dto.password, req.user.domain, dto.nickName, dto.status, dto.avatar, dto.email, dto.phoneNumber, dto.roleIds, req.user.uid));
    return ApiRes.ok();
  }
  ...
  async updateUser(@Body() dto: UserUpdateDto, @Request() req: any): Promise<ApiRes<null>> {
    await this.commandBus.execute(new UserUpdateCommand(dto.id, dto.username, dto.nickName, dto.status, dto.avatar, dto.email, dto.phoneNumber, dto.roleIds, req.user.uid));
    return ApiRes.ok();
  }
